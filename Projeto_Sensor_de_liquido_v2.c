...
void on_sensor(uint gpio, uint32_t events){ // 2- detecção do sensor
...
int main()
{
    ...
    gpio_set_irq_enabled_with_callback(SENSOR_PIN, GPIO_IRQ_EDGE_RISE | GPIO_IRQ_EDGE_FALL, true, on_sensor);

    while (true) {

        if (sensor_situation){ // 4- detectou, liga o led verde
            gpio_put(LED_RGB_RED_PIN,0);
